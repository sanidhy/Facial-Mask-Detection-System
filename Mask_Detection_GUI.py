...
def detect():
    ...
    PATH1 = r"D:\Python\OpenCV\Face_mask_detection\Face-Mask-Detection-master\face_detector\deploy.prototxt"
    PATH2 = r"D:\Python\OpenCV\Face_mask_detection\Face-Mask-Detection-master\face_detector\res10_300x300_ssd_iter_140000.caffemodel"
    faceNet = cv2.dnn.readNet(PATH1, PATH2)

    
    maskNet = load_model("D:\Python\OpenCV\Face_mask_detection\Face-Mask-Detection-master\mask_detector.model")
...
def Live_now():
    ...
    facecascade = cv2.CascadeClassifier("D:/OpenCV/FaceRecognition-master/HaarCascade/haarcascade_frontalface_default.xml")
    while True:
        ret,Frame=cap.read()
...
image_open=Image.open('D:/College Project/Screenshot/POSTER2.png')
resized=image_open.resize((width,height),Image.ANTIALIAS)
render=ImageTk.PhotoImage(resized)
img=Label(window,image=render)
img.place(x=0,y=0)

img1=Image.open('D:/College Project/Screenshot/POSTER1.jpg')
...
lbl_path = tk.Label(window, text="Enter path", width=20, height=2, fg="white", bg="purple2", font=('times', 15, ' bold '))
lbl_path.place(x=110,y=510)

path = tk.Entry(window, width=32, bg="snow",textvariable=folder_path, fg="black",font=('times', 25, ' bold '))
path.place(x=450, y=510)

browse = tk.Button(window, text="Browse",command=browse ,fg='white'  ,bg="purple2"  ,width=15  ,height=2 ,activebackground = "Red" ,font=('times', 15, ' bold '))
browse.place(x=200, y=600)

clearButton = tk.Button(window, text="Clear",command=clear,fg="white"  ,bg="purple2"  ,width=15  ,height=2 ,activebackground = "Red" ,font=('times', 15, ' bold '))
clearButton.place(x=800, y=600)

start_Button = tk.Button(window, text="Detect Now",command=detect_from_img ,fg="white"  ,bg="purple2"  ,width=15  ,height=2, activebackground = "Red" ,font=('times', 15, ' bold '))
start_Button.place(x=490, y=600)
